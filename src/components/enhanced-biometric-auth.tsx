...
import {
  useEnhancedBiometric,
  useEnhancedAnalytics,
  useEnhancedWallet,
} from "../providers/enhanced-minikit-provider";
import {
...
export function EnhancedBiometricAuth({
  ...
  const [selectedBiometric, setSelectedBiometric] =
    useState<BiometricType>("fingerprint");
  ...
  const [requireStrongAuthState, setRequireStrongAuth] =
    useState(requireStrongAuth);
  ...
  const checkBiometricAvailability = useCallback(async () => {
    try {
      ...
      setAuthStatus("unavailable");
      setAvailableBiometrics([]);
      logger.error("Failed to check biometric availability", {
        error: String(error),
      });
    }
  }, [biometric]);
  ...
  const loadAuthHistory = useCallback(async () => {
    try {
      ...
      if (stored) {
        const history = JSON.parse(stored).map(
          (item: {
            id: string;
            type: BiometricType;
            reason: string;
  ...
  const saveAuthToHistory = useCallback(
    (item: Omit<AuthHistoryItem, "id">) => {
  ...
  const handleAuthentication = useCallback(async () => {
    ...
    try {
      ...
      const options: BiometricAuthOptions = {
        ...
        subtitle:
          customPromptText || `Use ${selectedBiometric} to authenticate`,
        description: "Place your finger on the sensor or look at the camera",
        negativeButtonText: "Cancel",
      };

      const response = await biometric.authenticate(options);
  ...
  }, [showAuthHistory, loadAuthHistory]);
  ...
  // All authentication logic is handled in the handleAuthentication function above
    ...
    <div className={`enhanced-biometric-auth ${className}`}>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
        ...
        <div className="space-y-3">
          ...
          {/* Password Fallback */}
          {(showFallback ||
            (fallbackToPassword && authStatus === "failed")) && (
            <div className="border-t pt-4">
              ...
              <div className="flex space-x-2">
                <input
                  ...
                  onKeyPress={(e) =>
                    e.key === "Enter" && handlePasswordFallback()
                  }
                />
                <button
